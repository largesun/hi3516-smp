...
static int check_lane_id(combo_dev_t devno, input_mode_t input_mode, short p_lane_id[])
{
    ...
    else if (INPUT_MODE_LVDS == input_mode)
    {
...
static int check_lvds_wdr_mode(lvds_dev_attr_t* p_attr)
{
    ...
    switch(p_attr->wdr_mode)
    {
        ...
        case HI_WDR_MODE_4F:
        {
            if (LVDS_VSYNC_NORMAL != p_attr->vsync_attr.sync_type && LVDS_VSYNC_SHARE != p_attr->vsync_attr.sync_type)
            {
        ...
        {
            if (LVDS_VSYNC_NORMAL == p_attr->vsync_attr.sync_type)
            {
                if (LVDS_FID_IN_SAV != p_attr->fid_attr.fid_type && LVDS_FID_IN_DATA != p_attr->fid_attr.fid_type)
                {
                    HI_ERR("invalid fid_type, must be LVDS_FID_IN_SAV or LVDS_FID_IN_DATA\n");
                    ret = -1;
                }
            }
            else if (LVDS_VSYNC_HCONNECT == p_attr->vsync_attr.sync_type)
            {
                if (LVDS_FID_NONE != p_attr->fid_attr.fid_type && LVDS_FID_IN_DATA != p_attr->fid_attr.fid_type)
                {
...
static int check_lvds_dev_attr(combo_dev_t devno, lvds_dev_attr_t* p_attr)
{
    ...
    if (TRUE != p_attr->fid_attr.output_fil && FALSE != p_attr->fid_attr.output_fil)
    {
...
static int mipi_set_mipi_dev_attr(combo_dev_attr_t* p_combo_dev_attr)
{
    ...
    if (0 == mipi_dev_ctx.lane_divide_mode)
    {
        mipi_rx_drv_set_lane_num(devno, 4);
    }
    else
    {
        mipi_rx_drv_set_lane_num(devno, 2);

        /* 2l+2l mode, mipi ch1 only have 2 lane (0/1), need convert to (1/3) */
        if (1 == devno && 0x3 == lane_bitmap)
        {
            lane_bitmap = 0xa;
        }
    }
...
static int mipi_set_cmos_dev_attr(combo_dev_attr_t* p_combo_dev_attr)
{
    ...
    if ((devno > CMOS_MAX_DEV_NUM) || (0 == devno))
    {
    ...
    if (INPUT_MODE_BT656 != p_combo_dev_attr->input_mode)
    {
        if (1 == devno)
        {
...
static void proc_show_mipi_phy_data(osal_proc_entry_t* s)
{
    ...
    {
        osal_seq_printf(s, "%8d%8d,%2d,%2d,%2d    0x%02x,0x%02x,0x%02x,0x%02x    0x%02x,0x%02x,0x%02x,0x%02x    0x%02x,0x%02x,0x%02x,0x%02x\n",
                            i,
        ...
        for (lane = 0; lane < LVDS_LANE_NUM; lane++)
        {
            if (-1 != pstcombo_dev_attr->lvds_attr.lane_id[lane])
            {
                ...
                osal_seq_printf(s, "%6d%6d%8d%8d\n",
                                devno_array[devno_idx], pstcombo_dev_attr->lvds_attr.lane_id[lane], image_size.width, image_size.height);
            }
        }
    }
}
...
static void proc_show_mipi_int_err(osal_proc_entry_t* s, combo_dev_t devno_array[], int mipi_cnt)
{
    ...
    for (devno_idx = 0; devno_idx < mipi_cnt; devno_idx++)
    {
        ...
        osal_seq_printf(s, "%8d%15d%14d%7d%14d%15d\n",
                        ...
                        mipi_err_int_cnt[devno].vsync_cnt,
