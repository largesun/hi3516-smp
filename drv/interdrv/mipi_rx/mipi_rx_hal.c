...
static unsigned long mipi_rx_core_reset_addr;
...
void mipi_rx_drv_set_work_mode(combo_dev_t devno, input_mode_t input_mode)
{
    ...
    mipi_rx_work_mode_addr = (unsigned long)osal_ioremap_nocache(MIPI_RX_WORK_MODE_ADDR, (unsigned long)0x4);
    if (NULL == mipi_rx_work_mode_addr)
    {
    ...
    else if ((INPUT_MODE_SUBLVDS == input_mode)
             || (INPUT_MODE_LVDS == input_mode)
             || (INPUT_MODE_HISPI == input_mode))
    {
...
void mipi_rx_drv_set_di_dt(combo_dev_t devno, data_type_t input_data_type)
{
    ...
    else if (DATA_TYPE_YUV420_8BIT_NORMAL == input_data_type)
    {
        temp_data_type = 0x18;
    }
...
void mipi_rx_drv_set_phy_mode(mipi_rx_phy_t phy_id, input_mode_t input_mode, unsigned int lane_bitmap)
{
    ...
    if (INPUT_MODE_CMOS == input_mode
        || INPUT_MODE_BT1120 == input_mode
        || INPUT_MODE_BYPASS == input_mode)
    {
        cmos_en = 1;
    }
    ...
    else
    {
        ...
        else if (wdr_mode >= HI_WDR_MODE_DOL_2F && wdr_mode <= HI_WDR_MODE_DOL_4F)
        {
            ...
            if (LVDS_VSYNC_NORMAL == vsync_attr->sync_type)
            {
                ...
                if (LVDS_FID_IN_SAV == fid_attr->fid_type)
                {
                    lvds_wdr.bits.lvds_wdr_mode = 0x4;
                }
                else if (LVDS_FID_IN_DATA== fid_attr->fid_type)
                {
            ...
            else if (LVDS_VSYNC_HCONNECT== vsync_attr->sync_type)
            {
                ...
                if (LVDS_FID_NONE == fid_attr->fid_type)
                {
...
void set_lvds_sync_code(combo_dev_t devno, int n_frame, unsigned int lane_cnt,short lane_id[LVDS_LANE_NUM],
...
short get_sensor_lane_index(short lane, short lane_id[LVDS_LANE_NUM])
{
    int i = 0;
...
void mipi_rx_enable_disable_clock(combo_dev_t combo_dev, int enable)
{
    ...
    mipi_rx_clock_addr  = (unsigned long)osal_ioremap_nocache(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
    if (NULL == mipi_rx_clock_addr)
    {
...
void sensor_enable_disable_clock(sns_clk_source_t sns_clk_source, int enable)
{
    ...
    sensor_clock_addr  = (unsigned long)osal_ioremap_nocache(SNS_CRG_ADDR, (unsigned long)0x4);
    if (NULL == sensor_clock_addr)
    {
...
{
    set_bit(reset, (combo_dev+4), mipi_rx_core_reset_addr);
}
...
void sensor_reset_unreset(sns_rst_source_t sns_reset_source, int reset)
{
    ...
    if (0 == sns_reset_source)
    {
        offset = 1;
    }
    else if (1 == sns_reset_source)
    {
        offset = 7;
    }
    else
    ...
    sensor_reset_addr  = (unsigned long)osal_ioremap_nocache(SNS_CRG_ADDR, (unsigned long)0x4);
    if (NULL == sensor_reset_addr)
    {
...
static void lvds_int_statics(combo_dev_t devno)
{
    ...
    {
        lvds_err_int_cnt[devno].lvds_vsync_cnt++;
    }
    ...
    {
        lvds_err_int_cnt[devno].lane0_sync_err_cnt++;
    }

    if (lvds_ctrl_int & LANE1_SYNC_ERR)
    {
        lvds_err_int_cnt[devno].lane1_sync_err_cnt++;
    }

    if (lvds_ctrl_int & LANE2_SYNC_ERR)
    {
        lvds_err_int_cnt[devno].lane2_sync_err_cnt++;
    }

    if (lvds_ctrl_int & LANE3_SYNC_ERR)
    {
        lvds_err_int_cnt[devno].lane3_sync_err_cnt++;
    }
}
...
static int mipi_rx_drv_reg_init(void)
{
    if (!mipi_rx_regs_va)
    {
        mipi_rx_regs_va = (mipi_rx_regs_type_t *)osal_ioremap_nocache(MIPI_RX_REGS_ADDR, (unsigned int)MIPI_RX_REGS_SIZE);
        if (NULL == mipi_rx_regs_va)
        {
...
static void mipi_rx_drv_hw_init(void)
{
    ...
    mipi_rx_crg_addr = (unsigned long)osal_ioremap_nocache(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
...
int mipi_rx_drv_init(void)
{
    ...
    mipi_rx_core_reset_addr  = (unsigned long)osal_ioremap_nocache(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
    if (NULL == mipi_rx_core_reset_addr)
    {
...
void mipi_rx_drv_exit(void)
{
    ...
    mipi_rx_drv_reg_exit();
    mipi_rx_drv_hw_exit();
    osal_iounmap((void*)mipi_rx_core_reset_addr);
}
