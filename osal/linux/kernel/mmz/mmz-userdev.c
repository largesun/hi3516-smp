        ...
        struct mmb_info *pmi)
{
    ...
    if (mmb == NULL)
    {
        ...
        error("hil_mmb_alloc(%s, %llu, 0x%llx, %lu, %s) failed!\n",
              pmi->mmb_name, pmi->size, pmi->align,
              pmi->gfp, pmi->mmz_name);
#else
        error("hil_mmb_alloc(%s, %lu, 0x%lx, %lu, %s) failed!\n",
              pmi->mmb_name, pmi->size, pmi->align,
              pmi->gfp, pmi->mmz_name);
#endif
        return -ENOMEM;
...
static int mmz_userdev_ioctl_r(struct file *file, unsigned int cmd, struct mmb_info *pmi)
{
    switch(_IOC_NR(cmd)) {
        ...
        case _IOC_NR(IOC_MMB_DEC_REF):
            ...
            pmi->mmb_ref--;
            hil_mmb_put(pmi->mmb);
            if(pmi->delayed_free && 0==pmi->mmb_ref && 0==pmi->map_ref) {
...
static long mmz_userdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if(_IOC_TYPE(cmd) == 'm') {
        ...
        if(hil_vma_check ((unsigned long)area.dirty_virt_start, (unsigned long)area.dirty_virt_start + area.dirty_size))
        {
            printk(KERN_WARNING "\ndirty area[0x%lx,0x%lx] overflow!\n",
                (unsigned long)area.dirty_virt_start, (unsigned long)area.dirty_virt_start + area.dirty_size);
            ret = -EFAULT;
            up_read(&mm->mmap_sem);
            goto __error_exit;
        }
...
static int mmz_userdev_release(struct inode *inode, struct file *file)
{
    ...
    list_for_each_entry_safe(p, n, &pmu->list, list) {
        ...
        {
#if defined(KERNEL_BIT_64) && defined(USER_BIT_32)
            warning("mmb<0x%llx> mapped to userspace 0x%p will be unmaped!\n",
