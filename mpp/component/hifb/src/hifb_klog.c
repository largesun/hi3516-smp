...
#else
#define LOG_TRACE(fmt...)  printk(fmt)//HI_TRACE(HI_LOG_LEVEL_ERROR, HI_DEBUG_ID_FB, fmt)
#define LOG_ERROR(fmt...)  printk(fmt)//HI_TRACE(HI_LOG_LEVEL_ERROR, HI_DEBUG_ID_FB, fmt)
#endif
...
//HI_S32 Log_Show(struct seq_file *p, HI_VOID *v)
...
#define BUF_LEN 128
//HI_S32 Log_Write(struct file * file,
...
HI_S32 Log_Ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    if (HIGO_GET_LOG_MEM == cmd )
    {
        ...
        {
            //printk("phyaddr:0x%x, size:%d b\n", info.u32PhyAddr, info.u32Size);
