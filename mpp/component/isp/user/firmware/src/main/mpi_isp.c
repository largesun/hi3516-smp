...
HI_S32 HI_MPI_ISP_SetPubAttr(VI_PIPE ViPipe, const ISP_PUB_ATTR_S *pstPubAttr)
{
    ...
    if ((pstPubAttr->stWndRect.u32Width < RES_WIDTH_MIN) || (pstPubAttr->stWndRect.u32Width > RES_WIDTH_MAX(ViPipe)) || \
    ...
    if ((pstPubAttr->stWndRect.u32Height < RES_HEIGHT_MIN) || (pstPubAttr->stWndRect.u32Height > RES_HEIGHT_MAX(ViPipe)) || \
...
HI_S32 HI_MPI_ISP_GetFocusStatistics(VI_PIPE ViPipe, ISP_AF_STATISTICS_S *pstAfStat)
{
    ...
    if (HI_NULL == stActStatInfo.pVirtAddr )
    {
...
HI_S32 HI_MPI_ISP_GetAEStatistics(VI_PIPE ViPipe, ISP_AE_STATISTICS_S *pstAeStat)
{
    ...
    if ( HI_NULL == stIspStatInfo.pVirtAddr )
    {
        return HI_ERR_ISP_NULL_PTR;
    }
...
HI_S32 HI_MPI_ISP_SetStatisticsConfig(VI_PIPE ViPipe, const ISP_STATISTICS_CFG_S *pstStatCfg)
{
    ...
    if (ISP_MODE_RAW == stIspWorkMode.enIspDataInputMode)
    {
        //AWB
        ...
        if (ISP_AE_FOUR_PLANE_MODE_DISABLE == pstStatCfg->stAECfg.enFourPlaneMode && ISP_AE_HIST_SKIP_EVERY_PIXEL == pstStatCfg->stAECfg.stHistConfig.enHistSkipX)
        {
    ...
    if (0 != pstStatCfg->stFocusCfg.stConfig.stCrop.u16W % 8)
    {
        ISP_TRACE(HI_DBG_ERR, "Invalid Crop u16W:%d!\n", pstStatCfg->stFocusCfg.stConfig.stCrop.u16W);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }

    if (0 != pstStatCfg->stFocusCfg.stConfig.stCrop.u16H % 2)
    {
...
HI_S32 HI_MPI_ISP_GetStatisticsConfig(VI_PIPE ViPipe, ISP_STATISTICS_CFG_S *pstStatCfg)
{
    ...
    pstStatCfg->stFocusCfg.stConfig.bEnable = (0x0 != hi_ext_system_af_enable_read(ViPipe)) ? 1 : 0;
...
HI_S32 HI_MPI_ISP_SetDPCalibrate(VI_PIPE ViPipe, const ISP_DP_STATIC_CALIBRATE_S *pstDPCalibrate)
{
    ...
    if (HI_SUCCESS != ioctl(g_as32IspFd[ViPipe], ISP_GET_WDR_ATTR, &stWdrAttr))
    {
        ISP_TRACE(HI_DBG_ERR, "Get WDR attr failed\n");
        return HI_FAILURE;
    }

    if (pstDPCalibrate->bEnableDetect && IS_FS_WDR_MODE(stWdrAttr.enWDRMode))
    {
        ISP_TRACE(HI_DBG_ERR, "Can't enable dpc calibration under wdr mode!\n");
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
...
HI_S32 HI_MPI_ISP_SetDPStaticAttr(VI_PIPE ViPipe, const ISP_DP_STATIC_ATTR_S *pstDPStaticAttr)
{
    ...
    {
        if (1 == ISP_SUPPORT_OFFLINE_DPC_CALIBRATION)
        {
            u16StaticCountMax = u16StaticCountMax * stIspWorkMode.u8BlockNum;
        }
        else
...
HI_S32 HI_MPI_ISP_SetMeshShadingGainLutAttr(VI_PIPE ViPipe, const ISP_SHADING_LUT_ATTR_S *pstShadingLutAttr)
{
    ...
    for (i = 0; i < (HI_ISP_LSC_GRID_COL - 1) / 2; i++)
    {
        if ((pstShadingLutAttr->au16XGridWidth[i] > HI_ISP_LSC_XGRID_WIDTH_MAX(u32Width)) || (pstShadingLutAttr->au16XGridWidth[i] < 4))
        {
    ...
    for (i = 0; i < (HI_ISP_LSC_GRID_ROW - 1) / 2; i++)
    {
        if ((pstShadingLutAttr->au16YGridWidth[i] > HI_ISP_LSC_YGRID_WIDTH_MAX(u32Height)) || (pstShadingLutAttr->au16YGridWidth[i] < 4))
        {
...
HI_S32 HI_MPI_ISP_SetDemosaicAttr(VI_PIPE ViPipe, const ISP_DEMOSAIC_ATTR_S *pstDemosaicAttr)
{
    ...
    for (i = 0; i < ISP_AUTO_ISO_STRENGTH_NUM; i++)
    {
        ...
        if ((pstDemosaicAttr->stAuto.au8DetailSmoothRange[i] > HI_ISP_DEMOSAIC_DETAIL_SMOOTH_RANGE_MAX) || (pstDemosaicAttr->stAuto.au8DetailSmoothRange[i] < HI_ISP_DEMOSAIC_DETAIL_SMOOTH_RANGE_MIN))
        {
    ...
    if ((pstDemosaicAttr->stManual.u8DetailSmoothRange > HI_ISP_DEMOSAIC_DETAIL_SMOOTH_RANGE_MAX) || (pstDemosaicAttr->stManual.u8DetailSmoothRange < HI_ISP_DEMOSAIC_DETAIL_SMOOTH_RANGE_MIN))
    {
...
HI_S32 HI_MPI_ISP_GetDngImageStaticInfo(VI_PIPE ViPipe, DNG_IMAGE_STATIC_INFO_S *pstDngImageStaticInfo)
{
    ...
    if (HI_SUCCESS != ioctl(g_as32IspFd[ViPipe], ISP_DNG_INFO_GET, pstDngImageStaticInfo))
    {
        return HI_ERR_ISP_NOMEM;
    }
