...
HI_U32 log2_int_to_fixed(const HI_U32 val, const HI_U8 out_precision, const HI_U8 shift_out)
{
    ...
    //return (HI_U32)((((pos << precision) + result) << shift_out) | ((a & 0x7fff)>> (15-shift_out)));
    return (HI_U32)((((HI_U32)SignedLeftShift(pos, precision) + result) << shift_out) | ((a & 0x7fff) >> (15 - shift_out)));
}
    ...
    const HI_U8 shift_out)
{
    ...
    //return (HI_U32)(((pos << precision) + result) << shift_out);
...
HI_S64 SignedLeftShift(HI_S64 s64Value, HI_U8 u8BitShift)
{
    ...
    if (s64Value < 0)
    {
        s64Flag = s64Value & 0x8000000000000000L;
        return (HI_S64)(s64Flag | ((~(u64AbsValue << u8BitShift)) + 1));
    }
    else
    {
