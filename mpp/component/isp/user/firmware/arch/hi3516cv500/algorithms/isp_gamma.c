...
HI_VOID GammaGetPos(ISP_GAMMA_S *pstGamma, HI_U16 *pau16ActualLut, HI_U16 *pau16InputLut)
{
    ...
    HI_U16 i, j;
    HI_U16 InSegNum[GAMMA_OUTSEG_NUMBER];
    HI_U16 Point[GAMMA_OUTSEG_NUMBER];
    HI_U16 Step[GAMMA_OUTSEG_NUMBER];
    ...
    {
        LutPos[i] = LutPos[i] / 4;
        //LutPos[i] = CLIP_MAX(LutPos[i], GAMMA_NODE_NUMBER);
        pau16ActualLut[i] = pau16InputLut[LutPos[i]];
    }
    pau16ActualLut[GAMMA_NODE_NUMBER - 1] = 4095;
}
...
static HI_VOID GammaUsrRegsInit(ISP_GAMMA_USR_CFG_S *pstUsrRegCfg, HI_U16 *pu16Gamma, ISP_GAMMA_S *pstGamma)
{
    ...
    memcpy(pstUsrRegCfg->au16GammaLUT,   pu16Gamma,           GAMMA_REG_NODE_NUM   * sizeof(HI_U16));
}
...
static HI_S32 GammaCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_GAMMA_S *pstGamma)
{
    ...
    for (i = 0; i < GAMMA_EXT_NODE_NUMBER; i++)
    {
        ...
        {
            ISP_TRACE(HI_DBG_ERR, "Invalid au16Gamma[%d]!\n", i);
...
static HI_S32 GammaInitialize(VI_PIPE ViPipe)
{
    ...
    if (HI_NULL == pstGammaCtx->pu16GAMMA)
    {
    ...
    if (HI_NULL == pstGammaCtx->pu16ExtGAMMA)
    {
...
static HI_VOID ISP_GammaWdrModeSet(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++)
    {
        ...
        pstRegCfg->stAlgRegCfg[i].stGammaCfg.stUsrRegCfg.bSwitchLut2SttRegNew = HI_TRUE;
        pstRegCfg->stAlgRegCfg[i].stGammaCfg.stUsrRegCfg.u8SwitchRegNewCnt    = 0;
    }
}
                    ...
                    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (pstGammaCtx->bLutUpdate)
    {
        ...
        for (i = 0; i < pstReg->u8CfgNum; i++)
        {
            ...
            memcpy(pstUsrRegCfg->au16GammaLUT, pstGammaCtx->pu16GAMMA, GAMMA_REG_NODE_NUM * sizeof(HI_U16));
