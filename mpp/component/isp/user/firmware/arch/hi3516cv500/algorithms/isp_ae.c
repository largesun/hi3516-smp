...
static HI_S32 AeGetFrameInfo(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult)
{
    ...
    {
        pstIspFrame->au32Ratio[0] = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
...
static HI_VOID AeUpdateConfig(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    {
        pstAeRegCfg2->u64Exposure = (HI_U64)(pstAeResult->u32IntTime[0] >> 4) * pstAeResult->u32Iso * pstAeResult->u32PirisGain;
    }
    else
    {
        pstAeRegCfg2->u64Exposure = (HI_U64)(pstAeResult->u32IntTime[0] >> 4) * pstAeResult->u32Iso;
    }
}
...
static HI_VOID AeUpdateLinkage(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult, ISP_LINKAGE_S *pstLinkage)
{
    ...
    if (IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode))
    {
        ...
        pstLinkage->u32ExpRatio = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstLinkage->au32ExpRatio[0] = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
