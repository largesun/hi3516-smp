...
static HI_VOID BE_BlcDynaRegs_Linear(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, HI_U16 *pu16BlackLevel)
{
    ...
    for (j = 0;j < 4;j++)
    {
        ...
        {
            pstBeBlcCfg->st4DgBlc[i].stUsrRegCfg.au16Blc[j]     = pu16BlackLevel[j] << 2;/*4DG*/
        ...
        pstBeBlcCfg->stLscBlc.stUsrRegCfg.au16Blc[j]      = pu16BlackLevel[j] << 2;/*lsc*/
        pstBeBlcCfg->stDgBlc.stUsrRegCfg.au16Blc[j]       = pu16BlackLevel[j] << 2;/*Dg*/
        pstBeBlcCfg->stAeBlc.stUsrRegCfg.au16Blc[j]       = pu16BlackLevel[j] << 2;/*AE*/
        pstBeBlcCfg->stMgBlc.stUsrRegCfg.au16Blc[j]       = pu16BlackLevel[j] << 2;/*MG*/
        pstBeBlcCfg->stWbBlc.stUsrRegCfg.au16Blc[j]       = pu16BlackLevel[j] << 2;/*WB*/
    }
    /*bnr*/
    pstBeBlcCfg->stBnrBlc.stUsrRegCfg.au16Blc[0]  = pu16BlackLevel[0];//12bits
...
static HI_VOID BE_BlcDynaRegs_Wdr(VI_PIPE ViPipe, HI_U8 u8WDRModeState, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, HI_U16 *pu16BlackLevel)
{
    ...
    {
        WDROutBlc = ((pu16BlackLevel[0] >> 4) << 6);/*WDR outblc*/
        for (j = 0;j < 4;j++)
        {
            for (i = 0; i < ISP_WDR_CHN_MAX; i++)
            {
                pstBeBlcCfg->st4DgBlc[i].stUsrRegCfg.au16Blc[j]      = pu16BlackLevel[j] << 2;/*4DG*/
                pstBeBlcCfg->stWdrBlc[i].stUsrRegCfg.au16Blc[j]      = pu16BlackLevel[j] ;    /*WDR*/
                pstBeBlcCfg->stWdrBlc[i].stUsrRegCfg.u16OutBlc       = WDROutBlc;/*WDR*/
                pstBeBlcCfg->stFlickerBlc[i].stUsrRegCfg.au16Blc[j]  = pu16BlackLevel[j] << 2;/*flicker*/
            }
...
static HI_S32 BE_BlcDynaRegs_BuiltIn(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, HI_U16 *pu16BlackLevel)
{
    HI_U8  i, j;
    ...
    if (pstSnsDft->unKey.bit1Expander)
    {
        ...
        GetBuiltInExpanderBlc(pstSnsDft->pstExpander, pu16BlackLevel, au16BlackLevel);
    }
    else
    {
        for (i = 0; i < 4; i++)
    ...
    for (i = 0; i < 4; i++)
    {
        pstBlackLevel->au16ActualBlackLevel[i] = au16BlackLevel[i];/*Notice: Actual Blc is 14bits*/
    }
...
static HI_VOID BE_BlcDynaRegs(VI_PIPE ViPipe, HI_U8  u8WdrMode, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, HI_U16 *pu16BlackLevel)
{
    ...
    pstBeBlcCfg->stDeBlc.stUsrRegCfg.au16Blc[0]     = pu16BlackLevel[0];
...
static HI_VOID FE_BlcDynaRegs(ISP_FE_BLC_CFG_S  *pstFeBlcCfg, HI_U16 *pu16BlackLevel)
{
    HI_U8 j;
     ...
     for (j = 0;j < 4;j++)
    {
        pstFeBlcCfg->stFeDgBlc.stUsrRegCfg.au16Blc[j] = pu16BlackLevel[j] << 2;/*Fe Dg*/
        pstFeBlcCfg->stFeWbBlc.stUsrRegCfg.au16Blc[j] = pu16BlackLevel[j] << 2;/*Fe WB*/
        pstFeBlcCfg->stFeAeBlc.stUsrRegCfg.au16Blc[j] = pu16BlackLevel[j] << 2;/*Fe AE*/
    }
