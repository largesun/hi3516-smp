...
HI_S32 SAMPLE_RUNTIME_ForwardGroup_RFCN(const HI_CHAR* pcSrcFile, HI_RUNTIME_GROUP_HANDLE hGroupHandle)
{
    ...
    strncpy(astOutputBlob[0].acBlobName, "rois", MAX_NAME_LEN);
    ...
    strncpy(astOutputBlob[1].acBlobName, "cls_prob", MAX_NAME_LEN);
    ...
    strncpy(astOutputBlob[2].acBlobName, "bbox_pred", MAX_NAME_LEN);
    ...
    {
        HI_U32 u32BlobSize = stDst[i].u32Stride*stDst[i].unShape.stWhc.u32Height*stDst[i].unShape.stWhc.u32Chn*stDst[i].u32Num;
        memset((HI_VOID*)(HI_UL)(stDst[i].u64VirAddr), 0, u32BlobSize);
    }
